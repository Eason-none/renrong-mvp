...
// 前6个条目：跳过聊天，无Conversation
for (const itemId of ["color_001", "color_004", "color_005", "color_007", "color_010", "color_011"]) {
	completionEvent.createCompletionEvent({ contentId: itemId, contentType: "collection_item", collectionId: "collection_001" });
}

// 第7个条目：聊聊 + 主动归档（点开回顾前已有的摘要A）
...
// 第8个条目（最后一条→100%）：聊了但尚未归档——本变更的核心用例：
...
// ============================================================
memory.clear();
machine.activate("collection_006");
for (const itemId of ["city_001", "city_002", "city_003", "city_004", "city_006", "city_007", "city_008", "city_009", "city_010", "city_011"]) {
	completionEvent.createCompletionEvent({ contentId: itemId, contentType: "collection_item", collectionId: "collection_006" });
}
assertEqual(machine.getCollectionState("collection_006").status, "completed", "场景2前置: collection_006已达100%完成");
...
const [resultA, resultB] = await Promise.all([
	orchestration.ensureFirstReviewSnapshot("collection_006", slowGen, async () => null),
	orchestration.ensureFirstReviewSnapshot("collection_006", slowGen, async () => null),
]);
...
assertEqual(get(KEYS.REVIEW_SNAPSHOTS, []).filter((s) => s.collection_id === "collection_006").length, 1, "场景2【核心】: storage里最终只有1份快照");
...
memory.clear();
machine.activate("collection_001");
for (const itemId of ["color_001", "color_004", "color_005", "color_007", "color_008", "color_010", "color_011"]) {
	completionEvent.createCompletionEvent({ contentId: itemId, contentType: "collection_item", collectionId: "collection_001" });
}
...
memory.clear();
machine.activate("collection_006");
for (const itemId of ["city_001", "city_002", "city_003", "city_004", "city_006", "city_008", "city_009", "city_010", "city_011"]) {
	completionEvent.createCompletionEvent({ contentId: itemId, contentType: "collection_item", collectionId: "collection_006" });
}
const lastEvent = completionEvent.createCompletionEvent({ contentId: "city_007", contentType: "collection_item", collectionId: "collection_006" });
...
set(KEYS.REVIEW_SNAPSHOTS, [{
	id: "rs_legacy", collection_id: "collection_006", sequence: 1,
	generated_at: Date.now(), text: "旧时序遗留的快照文本", source_summary_ids: [],
}]);
assertEqual(machine.getCollectionState("collection_006").triggered_review_at_100pct, false, "场景5前置: 棘轮未置位的旧中间态");
...
assertEqual(machine.getCollectionState("collection_006").triggered_review_at_100pct, true, "场景5: 棘轮补置位，中间态收敛");
