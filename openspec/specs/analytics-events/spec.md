...
客户端匿名行为事件的采集与上报，为北极星指标（首用后 7 天内自发回访且再完成）和内容方向决策提供数据。设计基线：对用户完全不可感知、载荷零个人信息、失败安全（丢指标不丢功能）。指标口径与反指标清单见仓库根 `metrics.md`；决策背景见 `openspec/changes/archive/2026-07-08-add-metrics-system/`。
