# Spec: 丰容（Enrichment）MVP — v1【已归档】

> 📁 **归档说明（2026-07-12 文档收编）**：本文件已从仓库根目录移入 `docs/archive/`，
> **不再是任何功能的实现依据**。保留的唯一原因：代码注释多处引用其章节号
> （如 `docs/archive/spec_v1.md §2.4`），用于回溯首版实体定义与验收标准的措辞。
> 当前权威顺序见 `product_handoff.md` 文件头。
>
...
> ⚠️ **时效警告（2026-07-06 添加）**：本 spec 基于 handoff **v5**，以下机制已被后续设计取代且实现已按新设计落地，**请勿以本文件相关章节为准**：
> - 名额体系 / 锁回去 / 4 个上限 / 50% 发名额 → v7 定稿：固定 **3 个激活上限 + 随时放下**，名额概念整体移除（handoff §5.3）
> - 场景三分类 室内短/室内久/室外 → v7 定稿：11 维 scene_tags（handoff §6.4）；PushFlow/pushPool/GlobalDoneSet 已于 2026-07-06 remove-pushflow 变更整体删除，本文件 §2.3、§3.1 所述推送层机制不复存在
> - 归档按钮 + 5 条消息弹窗 → v8 定稿：常驻"说完了"按钮 + 固定收尾文案（handoff §6.5.3）
>
> 其余部分（对话 1:1 隔离、摘要生成、回顾快照定格时序、完成度棘轮）仍可作为首版措辞参考，但一切以 `openspec/specs/` 为准。
...
- **AC3**：Given 用户点进"丰容探索"图鉴卡片提示但回顾仍在生成中，Then 展示邀请式加载文案（非系统进度条语气）。
